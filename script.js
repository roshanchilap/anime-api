...
searchResults.innerHTML= data.results.map(data=>{
    return `
  ...
  <div class="card-body">
    ...
    <p class="card-rating"><span class="sp1">IMDB rating:</span> ⭐${data.score}</p>
    <p class="card-sdate"><span class="sp1">Start date:</span> ${new Date(data.start_date).toDateString()}</p>
    <p class="card-edate"><span class="sp1">End date:</span> ${new Date(data.end_date).toDateString()}</p>
  </div>
</div>
    `
}).join('')
